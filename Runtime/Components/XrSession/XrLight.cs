...
namespace SturfeeVPS.SDK
{
    ...
    public class XrLight : MonoBehaviour
    {
        ...
        {
            _latitude = XrSessionManager.GetSession().Location.Latitude;
            _longitude = XrSessionManager.GetSession().Location.Longitude;
            _time = DateTime.Now;
    ...
    */
        ...
        private void ManageLayers()
        {
            ...
            // Building
            layer = LayerMask.NameToLayer(SturfeeLayers.SturgBuilding);
            if ((light.cullingMask & (1 << layer)) == 0)
            {
                light.cullingMask |= 1 << LayerMask.NameToLayer(SturfeeLayers.SturgBuilding);
            }

            // Terrain
            layer = LayerMask.NameToLayer(SturfeeLayers.SturgTerrain);
            if ((light.cullingMask & (1 << layer)) == 0)
            {
                light.cullingMask |= 1 << LayerMask.NameToLayer(SturfeeLayers.SturgTerrain);
            }
        }
    }
}